...
class RankSubFragment : BaseLazyFragment() {
    ...
    override fun initView() {
        ...
        val adapter = LRecyclerViewAdapter(object : AutoCommonAdapter<BklistItem.Item>(activity, R.layout.item_novel, data) {
            override fun convert(holder: com.zhy.adapter.recyclerview.base.ViewHolder?, t: BklistItem.Item?, position: Int) {
                ...
                //LogHelper.LogE(t)
                Glide.with(this@RankSubFragment).load(t.img).into(holder.convertView.ivImg)
    ...
    companion object {
        fun newInstance(type: String): RankSubFragment {
            val homeFragment = RankSubFragment()
    ...
    private fun loadData(isFirst: Boolean, isRefresh: Boolean) {
        HttpMethods.instance.rank(object : BaseObserver<BklistItem>(activity) {
    ...
    fun getUrl(): String {
        return "${APIService.BASEURL}/bqgph/${type}_$currentPageNum.html"
    }
}
